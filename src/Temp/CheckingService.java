                ...
                if (rs.getString("temp_c_val") != null) {
                    ...
                    if (temp < low || temp > high) {
                        if(!outOfRange){
                            ...
                            lastAlert = temp;
                            if(temp<low){
                                Contacts.sendTextMessage(con,"The temperature has decreased below range at "+temp, "Temp too low" );

                                System.out.println("Temp just became too low at "+temp);
                            }else{
                                Contacts.sendTextMessage(con,"The temperature has increased above range at "+temp, "Temp too high");

                                System.out.println("Temp just became too high at "+temp);
                            }
                        }else{
                            if (temp < low && (lastAlert - temp) > 5) {
                                Contacts.sendTextMessage(con,"The temperature has decreased from "+lastAlert+" to "+temp, "Temp too low" );
                                System.out.println("Temp is decreased from " + lastAlert + " to " + temp);
                            } else if (temp > high && (temp - lastAlert) > 5) {
                                Contacts.sendTextMessage(con,"The temperature has increased from "+lastAlert+" to "+temp, "Temp too high");
                        ...
                        if (outOfRange) {
                            Contacts.sendTextMessage(con,"The temperature is acceptable at "+temp, "Temp stabilized");
